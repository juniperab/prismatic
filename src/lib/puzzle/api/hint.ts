...
const getHintItemsAsArrayVisitor: HintVisitor<(HintItem | undefined)[]> = {
...
export function visitHintItems<T>(hint: Hint, visitor: (hintItem: HintItem) => T): (T | undefined)[] {
    const items = visitHint(hint, getHintItemsAsArrayVisitor)
    return items.map(item => item ? visitor(item) : undefined)
}
