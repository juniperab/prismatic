...
function renderOptionsItemContents<T>(option: T, selected: T | undefined, formatter?: (opt: T) => string): ReactElement {
...
export function OptionsOneLine<T>(props: OptionsOneLineProps<T>): ReactElement {
  ...
  const items = options.map((opt, idx) => {
    return <OptionsItem key={idx} onClick={() => onSelect(opt)}>
      {renderOptionsItemContents(opt, selected, optionFormatter)}
    </OptionsItem>
  })

  return <OptionsOneLineOuter>
