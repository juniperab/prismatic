...
export function HintGrid(props: HintGridProps): ReactElement {
  const { answer, hints, hintStyle, maxHeight, maxWidth, numCols, numRows, onClick } = props
  ...
  while (items.length < maxItems) items.push(<HintCircle hintStyle={hintStyle} key={items.length} />)
