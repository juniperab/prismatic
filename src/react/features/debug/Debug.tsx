...
export function Debug(): ReactElement {
  ...
  const { currentColour, hints, mode, precision, puzzleId } =
    useAppSelector(selectPuzzleState)
  ...
  function swatch(colour: AnyColor): ReactElement {
    ...
    return (
      <span
        className="swatch"
        style={{ backgroundColor: hex, color: swatchTextColour }}
      >
        &nbsp;{hex}&nbsp;
      </span>
    )
  ...
  function describeColour(colour: AnyColor): ReactElement {
    ...
    return (
      <ul>
        <li>
          Hex: {swatch(rgb)} RGB: {rgb.r}, {rgb.g}, {rgb.b}
        </li>
        <li>
          HSL/V: {hsl.h} {hsl.s}% {hsl.l}% / {hsb.h} {hsb.s}% {hsb.b}%
        </li>
      </ul>
    )
  }
      ...
      <>
        ...
        <ul style={{ fontSize: 'xx-small' }}>
          {hints.map((hint: Hint, idx: number) => {
            ...
            const hslDiffString =
              'ΔHSL: ' +
              `${hueDiff(hslT.h, hslG.h)} ${hslT.s - hslG.s}% ${
                hslT.l - hslG.l
              }%`
            const hsbDiffString =
              'HSB: ' +
              `${hueDiff(hsbT.h, hsbG.h)} ${hsbT.s - hsbG.s}% ${
                hsbT.b - hsbG.b
              }%`
            let diffString = ''
            switch (displayMode) {
              case 'rgb':
                diffString = rgbDiffString
                break
              case 'hsl':
                diffString = hslDiffString
                break
              case 'hsb':
                diffString = hsbDiffString
                break
            }
            return (
              <li key={idx}>
